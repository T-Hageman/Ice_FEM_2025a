...
Plot_Surf3D::Plot_Surf3D(std::string name, inputData &inputs, Physics *phys): Vis_Plot(name, inputs, phys) {
...
Plot_Surf3D::~Plot_Surf3D(){

}

void Plot_Surf3D::Plot(){
    ...
    if (Deformations){
        // Get deformation mesh coordinates to ensure consistency
        std::vector<std::vector<double>> XCoords_u, YCoords_u, ZCoords_u, XIPCoords_u, YIPCoords_u, ZIPCoords_u;
        size_t nNodes_per_Elem_u = physics->mesh->GetExportMeshCoords(ElemGroupIndex_u, XCoords_u, YCoords_u, ZCoords_u, XIPCoords_u, YIPCoords_u, ZIPCoords_u);
        size_t nElems_u = XCoords_u.size();
        
        // Ensure deformation data matches visualization data dimensions
        if (nElems_u != nElems || nNodes_per_Elem_u != nNodes_per_Elem) {
            throw std::runtime_error("Deformation element group has different dimensions than data element group. nElems_u=" + 
                                   std::to_string(nElems_u) + " vs nElems=" + std::to_string(nElems) + 
                                   ", nNodes_u=" + std::to_string(nNodes_per_Elem_u) + " vs nNodes=" + std::to_string(nNodes_per_Elem));
        }
        ...
        deformZ.resize(nElems); 
        for (size_t i = 0; i < nElems; i++) deformZ[i].resize(nNodes_per_Elem);
        ...
        physics->GetNodalDataToSave(DofNames_u[2], ElemGroupIndex_u, deformZ);
    }
    ...
    DataToUGrid(XCoords, YCoords, ZCoords, NodeData, deformX, deformY, deformZ, colorLookupTable, false);
    ...
    if (rank==0){
        ...
        if (DoOnce){
            ...
            ZRange[0] = ugridMapper->GetBounds()[4];
            ZRange[1] = ugridMapper->GetBounds()[5];
...
void Plot_Surf3D::DataToUGrid(std::vector<std::vector<double>> &XCoords, std::vector<std::vector<double>> &YCoords, std::vector<std::vector<double>> &ZCoords, 
    ...
    if (size==1){ //can directly add points, no need to check ranks
        ...
        for (size_t i = 0; i < nElems; i++){
            for (size_t j = 0; j < nNodes_per_Elem; j++){
                if (UpdateCOnly==false){
                    if (Deformations){
                        x[0] = XCoords[i][j]+DeformationScale*UX[i][j];
                        x[1] = YCoords[i][j]+DeformationScale*UY[i][j];
                        x[2] = ZCoords[i][j]+DeformationScale*UZ[i][j];
                    } else {
                        x[0] = XCoords[i][j];
                        x[1] = YCoords[i][j];
                        x[2] = ZCoords[i][j];
                    ...
                    points->InsertPoint(ptCounter, x);
                    connects[order[j]] = ptCounter;
                }
                c = Data[i][j];
            ...
            for (size_t j = 0; j < nNodes_per_Elem; j++){
                if (UpdateCOnly==false){
                    if (Deformations){
                        ...
                        x2[ptCounter] = ZCoords[i][j]+DeformationScale*UZ[i][j];
                    } else {
                        x0[ptCounter] = XCoords[i][j];
                        x1[ptCounter] = YCoords[i][j];
                        x2[ptCounter] = ZCoords[i][j];
                    }
        ...
        if(rank==0){
            ...
            for (size_t core = 0; core < size; core++){
                ...
                if (NPoints[core]>0){
                    size_t currentElement = connects_i[0];
                    for (size_t i = 0; i < NPoints[core]; i++){
                        if (UpdateCOnly==false){
                            x[0] = x0[i];
                            x[1] = x1[i];
                            ...
                            // Check if this is the last point of current element or if we've moved to next element
                            bool isLastPointOfElement = false;
                            if (i == NPoints[core]-1) {
                                isLastPointOfElement = true; // Last point overall
                            } else if (connects_i[i+1] != connects_i[i]) {
                                isLastPointOfElement = true; // Next point belongs to different element
                            }
                            
                            // Add cell when we finish processing all nodes of current element
                            if (isLastPointOfElement) {
                                bool add = true;
                                for (size_t j = 0; j < nNodes_per_Elem; j++){
                                    if (connects[j] > totalPoints){ //fix for indexing error
                                        add = false;
                                    }
                                }
                                if (add){
                                ...
                                currentElement = (i < NPoints[core]-1) ? connects_i[i+1] : currentElement;
                            }
                        }
