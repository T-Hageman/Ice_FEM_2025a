#include "NURBS3_Plane1.h"
#include "../../../utility/utility.h"

NURBS3_Plane1::NURBS3_Plane1(int ipcount1D){
    Name = "NURBS3_Plane1";
    NodeCount = 4;
    requiresData = true;
...
NURBS3_Plane1::~NURBS3_Plane1(){

};

void NURBS3_Plane1::SetUpBaseShapes(){
    Nbase.resize(ipcount);
    Gbase.resize(ipcount);
    G2base.resize(ipcount);

    Eigen::VectorXd Nx(2), Ny(2);
    Eigen::VectorXd Gx(2), Gy(2);
    Eigen::VectorXd G2x(2), G2y(2);
...
void NURBS3_Plane1::getShapeGrads(std::vector<Eigen::RowVectorXd> &Nout, std::vector<Eigen::MatrixXd> &Gout, std::vector<double> &wout, Eigen::MatrixXd &coordsNodes, Eigen::MatrixXd &ElemData, Eigen::VectorXd &NodeData){
    Eigen::MatrixXd J(2,3);
    Eigen::RowVector3d tang1, tang2, normal;

    Eigen::DiagonalMatrix<double, 4> W(NodeData);
...
void NURBS3_Plane1::getCoordsIP(Eigen::MatrixXd &coordsIP, Eigen::MatrixXd &coordsNodes, Eigen::MatrixXd &ElemData, Eigen::VectorXd &NodeData){
    Eigen::DiagonalMatrix<double, 4> W(NodeData);
    Eigen::VectorXd CB(NodeCount);
    double wFun;
...
void NURBS3_Plane1::getExportShape(std::vector<Eigen::RowVectorXd> &Nout, Eigen::MatrixXd &ElemData, Eigen::VectorXd &NodeData){
    Eigen::DiagonalMatrix<double, 4> W(NodeData);
    Eigen::VectorXd CB(NodeCount);
    double wFun;
...
void NURBS3_Plane1::getNormals(std::vector<Eigen::VectorXd> &normals, Eigen::MatrixXd &coordsNodes, Eigen::MatrixXd &ElemData, Eigen::VectorXd &NodeData){
    Eigen::MatrixXd J(2,3);
