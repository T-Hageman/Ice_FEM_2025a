#include "NURBS3_Cube1.h"
#include "../../../utility/utility.h"

NURBS3_Cube1::NURBS3_Cube1(int ipcount1D){
    Name = "NURBS3_Cube1";
    NodeCount = 2*2*2;
    requiresData = true;
...
NURBS3_Cube1::~NURBS3_Cube1(){

};

void NURBS3_Cube1::SetUpBaseShapes(){
    Nbase.resize(ipcount);
    Gbase.resize(ipcount);
    G2base.resize(ipcount);

    Eigen::VectorXd Nx(2), Ny(2), Nz(2);
    Eigen::VectorXd Gx(2), Gy(2), Gz(2);
    Eigen::VectorXd G2x(2), G2y(2), G2z(2);
    for (size_t i = 0; i < ipcount; i++){
        Eigen::Vector3d coords = coordsIP[i];

        double x = coords(0), y = coords(1), z = coords(2);
        BezierBasis(1,x,Nx,Gx,G2x);
        BezierBasis(1,y,Ny,Gy,G2y);
        BezierBasis(1,z,Nz,Gz,G2z);
...
void NURBS3_Cube1::getShapeGrads(std::vector<Eigen::RowVectorXd> &Nout, std::vector<Eigen::MatrixXd> &Gout, std::vector<double> &wout, Eigen::MatrixXd &coordsNodes, Eigen::MatrixXd &ElemData, Eigen::VectorXd &NodeData){
    Eigen::Matrix3d J, Jinv;
    Eigen::DiagonalMatrix<double, 8> W(NodeData);
...
void NURBS3_Cube1::getShapeGrads2(std::vector<Eigen::RowVectorXd> &Nout, std::vector<Eigen::MatrixXd> &Gout, std::vector<Eigen::MatrixXd> &G2out, std::vector<double> &wout, Eigen::MatrixXd &coordsNodes, Eigen::MatrixXd &ElemData, Eigen::VectorXd &NodeData){
    Eigen::Matrix3d J, Jinv;
...
void NURBS3_Cube1::getCoordsIP(Eigen::MatrixXd &coordsIP, Eigen::MatrixXd &coordsNodes, Eigen::MatrixXd &ElemData, Eigen::VectorXd &NodeData){
    Eigen::DiagonalMatrix<double, 8> W(NodeData);
    Eigen::VectorXd CB(NodeCount);
    double wFun;
    ...
    for (size_t i = 0; i < ipcount; i++){
        ...
        coordsIP.row(i) = (W*CB/wFun).transpose()*coordsNodes;
    }
}

void NURBS3_Cube1::getExportShape(std::vector<Eigen::RowVectorXd> &Nout, Eigen::MatrixXd &ElemData, Eigen::VectorXd &NodeData){
    Eigen::DiagonalMatrix<double, 8> W(NodeData);
    Eigen::VectorXd CB(NodeCount);
    double wFun;
