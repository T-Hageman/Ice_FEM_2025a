#include "NURBS3_Cube3.h"
#include "../../../utility/utility.h"

NURBS3_Cube3::NURBS3_Cube3(int ipcount1D){
    Name = "NURBS3_Cube3";
    NodeCount = 4*4*4;
    requiresData = true;
...
NURBS3_Cube3::~NURBS3_Cube3(){

};

void NURBS3_Cube3::SetUpBaseShapes(){
    Nbase.resize(ipcount);
    Gbase.resize(ipcount);
    G2base.resize(ipcount);

    Eigen::VectorXd Nx(4), Ny(4), Nz(4);
    Eigen::VectorXd Gx(4), Gy(4), Gz(4);
    Eigen::VectorXd G2x(4), G2y(4), G2z(4);
...
void NURBS3_Cube3::getShapeGrads(std::vector<Eigen::RowVectorXd> &Nout, std::vector<Eigen::MatrixXd> &Gout, std::vector<double> &wout, Eigen::MatrixXd &coordsNodes, Eigen::MatrixXd &ElemData, Eigen::VectorXd &NodeData){
    Eigen::Matrix3d J, Jinv;
    Eigen::DiagonalMatrix<double, 64> W(NodeData);
...
void NURBS3_Cube3::getShapeGrads2(std::vector<Eigen::RowVectorXd> &Nout, std::vector<Eigen::MatrixXd> &Gout, std::vector<Eigen::MatrixXd> &G2out, std::vector<double> &wout, Eigen::MatrixXd &coordsNodes, Eigen::MatrixXd &ElemData, Eigen::VectorXd &NodeData){
    Eigen::Matrix3d J, Jinv;
...
void NURBS3_Cube3::getCoordsIP(Eigen::MatrixXd &coordsIP, Eigen::MatrixXd &coordsNodes, Eigen::MatrixXd &ElemData, Eigen::VectorXd &NodeData){
    Eigen::VectorXd CB(NodeCount);
    double wFun;
    Eigen::DiagonalMatrix<double, 64> W(NodeData);
...
void NURBS3_Cube3::getExportShape(std::vector<Eigen::RowVectorXd> &Nout, Eigen::MatrixXd &ElemData, Eigen::VectorXd &NodeData){
    Eigen::VectorXd CB(NodeCount);
    double wFun;
    Eigen::DiagonalMatrix<double, 64> W(NodeData);
